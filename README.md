...
# 3. 启动开发服务器（前端 :5180 HTTPS + 后端 :8003，端口被占自动 +1，最多试 10 次）
make dev
```

浏览器打开 `https://localhost:5180`（被占时启动日志会打印实际端口，如 `:5181`），首次访问会弹出花名输入框。
...
make dev            # 同时启动前端 :5180 (HTTPS) + 后端 :8003，端口被占自动 +1（上限 10 次）
