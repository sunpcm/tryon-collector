...
- **与早期 UI 草图的关系**（`docs/archive/2.md`）：草图中的四卡槽交互作为「单任务精修模式」保留为矩阵的一行展开视图；主流程升级为批量矩阵。
...
### 2.1 任务包 (Task Bundle)
一次「设计师 + 一款商品」的最小提交单元。一个 Bundle 必包含 `product` / `tryon` / `retouched` 三类图各 1 张，可选 `annotated` 1 张。后端落盘后生成 `<uuid>/` 目录与 `metadata.json`（结构沿用早期系统设计 §三，见 `docs/archive/1.md`）。
...
- 每行可展开为四格沉浸式卡槽视图（见 `docs/archive/2.md`），用于逐张替换/粘贴。
...
- `metadata.json` 结构遵循早期系统设计 §三（`docs/archive/1.md`），新增字段：`group_key`、`client_submit_id`（幂等键）、`file_hashes`。
...
- 侧边栏 Gamification（见 `docs/archive/2.md`）：读取本机提交计数，展示「拦截 AI 翻车 N 次」。
- 聚类兜底策略开关（§2.3 第 2 条）。
