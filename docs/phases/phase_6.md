...
问题：`ManualSubmitBar` 只取每个角色的第一张图（`roleFiles[0]`），多上传的图片被静默丢弃。

修复：改为按索引配对，每个角色的每张图都生成独立 bundle（groupKey = UUID）。例如产品图 3 张、试穿图 2 张、精修图 2 张 → 生成 3 个 bundle，前 2 个完整，第 3 个仅含产品图（后端 rejected）。
